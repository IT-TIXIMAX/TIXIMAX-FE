...
const DashBoardWarehouseA = () => {
  const currentYear = new Date().getFullYear();

  const [year, setYear] = useState(currentYear);
  const [data, setData] = useState([]); // [{month,totalWeight}]
  ...
  const fetchYearly = async (y) => {
    try {
      setLoading(true);
      setError("");
      // ✅ bạn cần có service: getYearlyWarehouseA / getYearlyWarehouseWeight ...
      // Nếu API bạn đang dùng trả về [{month,totalWeight}] thì gọi đúng hàm tương ứng:
      // Ví dụ: dashboardService.getYearlyWarehouseA(y)
      const res = await dashboardService.getYearlyWarehouse(y);
      ...
      setError(
        e?.response?.data?.message ||
          e?.message ||
          "Không thể tải dữ liệu WarehouseA (Weight)."
      );
      setData([]);
    } finally {
      setLoading(false);
  ...
  // ✅ đảm bảo đủ 12 tháng + đọc đúng field totalWeight
  const fullYearData = useMemo(() => {
    ...
    return Array.from({ length: 12 }, (_, i) => {
      ...
      const totalWeight = Number(found?.totalWeight ?? 0);
      return {
        month: m,
  ...
  const bestMonth = useMemo(() => {
    ...
    return fullYearData.reduce(
      (best, cur) => (cur.totalWeight > (best?.totalWeight ?? -1) ? cur : best),
      null
    );
  }, [fullYearData]);

  const activeMonthsCount = useMemo(() => {
    return fullYearData.filter((x) => x.totalWeight > 0).length;
  }, [fullYearData]);
      ...
      datasets: [
        {
          ...
          data: fullYearData.map((x) => x.totalWeight),
  ...
  const chartOptions = useMemo(() => {
    ...
    return {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `WarehouseA - Tổng trọng lượng theo tháng (${year})`,
          color: "#facc15",
          font: { size: 18 },
        },
        tooltip: {
          callbacks: {
            label: (ctx) => {
              ...
              return `Tổng trọng lượng: ${nf.format(v)} kg`;
            },
          },
        },
      },
  ...
  const metricCards = useMemo(() => {
    ...
    return [
      {
        title: "Tổng trọng lượng (năm)",
        value: `${nf.format(totals.weight)} kg`,
        icon: <Scale />,
        ariaLabel: "Tổng trọng lượng trong năm",
      },
      {
        title: "Số tháng có dữ liệu",
        value: String(activeMonthsCount),
        icon: <Package />,
        ariaLabel: "Số tháng có phát sinh trọng lượng",
      },
      {
        title: "Năm đang xem",
        value: String(year),
      ...
      },
      {
        ...
        value: bestMonth
          ? `${monthName(bestMonth.month)} • ${nf.format(
              bestMonth.totalWeight
            )} kg`
          : "N/A",
        icon: <TrendingUp />,
        ariaLabel: "Tháng có trọng lượng cao nhất",
      },
    ];
  }, [totals.weight, activeMonthsCount, year, bestMonth]);
      ...
      <div className="mx-auto ">
        ...
        <section className="mb-12">
          <h2 className="text-2xl font-semibold text-yellow-300 mb-6">
            Biểu đồ trọng lượng theo tháng
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6 mb-12">
          {loading ? (
            ...
            fullYearData.map((m) => {
              const isActive = m.totalWeight > 0;
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        >
                          {isActive ? "Có phát sinh" : "Chưa có dữ liệu"}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="mt-4">
                    <p className="text-sm text-gray-300 flex items-center gap-2">
                      <Scale className="w-4 h-4 text-green-400" aria-hidden />
                      Tổng trọng lượng:{" "}
                      <span className="font-semibold text-gray-100">
                        {new Intl.NumberFormat("vi-VN", {
                          maximumFractionDigits: 2,
                        }).format(m.totalWeight)}{" "}
                        kg
                      </span>
                    </p>
                  </div>
...
export default DashBoardWarehouseA;
