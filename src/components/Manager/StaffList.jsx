...
/* ===================== StaffTimeSection ===================== */
const StaffTimeSection = ({ data, loading, onRefresh }) => (
  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
    <div className="flex items-center justify-between mb-4">
      ...
      <button
        onClick={onRefresh}
        disabled={loading}
        className="p-1.5 rounded-lg border border-gray-200 text-gray-400 hover:bg-gray-50 disabled:opacity-50 transition-colors"
        type="button"
        title="Tải lại"
      >
    ...
    {loading ? (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
        {[...data]
          ...
          .map((item, idx) => {
            const maxCount = Math.max(...data.map((d) => d.customerCount), 1);
            const pct = Math.round((item.customerCount / maxCount) * 100);
            const c = STAFF_CARD_COLORS[idx % STAFF_CARD_COLORS.length];
            return (
              <div
                key={item.staffName}
              ...
              >
                <p
                  className="text-xl font-medium text-gray-600 mb-1 truncate"
                  title={item.staffName}
                >
                  {item.staffName}
                </p>
                <p className={`text-2xl font-bold ${c.text} leading-tight`}>
                  {item.customerCount.toLocaleString("vi-VN")}
                </p>
                <p className="text-xl text-gray-400 mt-0.5">khách hàng mới</p>
                <div className="mt-3 h-1.5 bg-white/60 rounded-full overflow-hidden border border-gray-200">
                  <div
...
const StaffList = () => {
  ...
  // ✅ Staff time customers
  const [staffTimeData, setStaffTimeData] = useState([]);
  const [staffTimeLoading, setStaffTimeLoading] = useState(false);
  ...
  // ✅ Fetch staff time customers
  const fetchStaffTimeCustomers = useCallback(async () => {
    setStaffTimeLoading(true);
    try {
      const data = await dashboardService.staffTimeCustomers();
      setStaffTimeData(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error(
        "Không thể tải dữ liệu khách hàng mới theo nhân viên:",
        err,
      );
    } finally {
      setStaffTimeLoading(false);
    }
  }, []);
  ...
  // ✅ Load staff time customers on mount
  useEffect(() => {
    fetchStaffTimeCustomers();
  }, [fetchStaffTimeCustomers]);
  ...
  /* ===================== Utils ===================== */
  const getRoleInfo = useCallback(
    (role) => roleMap.get(role) || { label: role || "—", color: "gray" },
    [roleMap],
  );
  ...
  return (
    <div className="min-h-screen">
      <div className="mx-auto p-4 md:p-6 lg:p-8">
        ...
        <StaffTimeSection
          data={staffTimeData}
          loading={staffTimeLoading}
          onRefresh={fetchStaffTimeCustomers}
        />
                ...
                <div className="flex gap-2">
                  {PAGE_SIZES.map((size) => (
                    <button
                      key={size}
                      onClick={() => handlePageSizeChange(size)}
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${pageSize === size ? "bg-blue-600 text-white shadow-sm" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
                      type="button"
                    >
                      {size}
                    </button>
                  ...
                  {filteredStaff.map((staff, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-4">
                          <span
                            className={`inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-full ${getRoleColor(roleInfo.color)}`}
                          >
                            {roleInfo.label}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-4">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full ${getStatusBadge(staff.status)}`}
                          >
                            {isActive ? (
                              <Unlock className="w-3 h-3" />
                            ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      value={formData.username}
                      onChange={handleInputChange}
                      className={`w-full border-2 rounded-lg px-3 py-2.5 outline-none focus:ring-0 transition-all ${formErrors.username ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-blue-500"}`}
                      placeholder="username"
                    />
                    {formErrors.username && (
                      <p className="text-sm text-red-600 mt-1 flex items-center gap-1">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={formData.password}
                      onChange={handleInputChange}
                      className={`w-full border-2 rounded-lg px-3 py-2.5 outline-none focus:ring-0 transition-all ${formErrors.password ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-blue-500"}`}
                      placeholder="••••••••"
                    />
                    {formErrors.password && (
                      <p className="text-sm text-red-600 mt-1 flex items-center gap-1">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={formData.confirmPassword}
                      onChange={handleInputChange}
                      className={`w-full border-2 rounded-lg px-3 py-2.5 outline-none focus:ring-0 transition-all ${formErrors.confirmPassword ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-blue-500"}`}
                      placeholder="••••••••"
                    />
                    {formErrors.confirmPassword && (
                      <p className="text-sm text-red-600 mt-1 flex items-center gap-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      value={formData.name}
                      onChange={handleInputChange}
                      className={`w-full border-2 rounded-lg px-3 py-2.5 outline-none focus:ring-0 transition-all ${formErrors.name ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-blue-500"}`}
                      placeholder="Nguyễn Văn A"
                    />
                    {formErrors.name && (
                      <p className="text-sm text-red-600 mt-1 flex items-center gap-1">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={formData.email}
                      onChange={handleInputChange}
                      className={`w-full border-2 rounded-lg px-3 py-2.5 outline-none focus:ring-0 transition-all ${formErrors.email ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-blue-500"}`}
                      placeholder="email@example.com"
                    />
                    {formErrors.email && (
                      <p className="text-sm text-red-600 mt-1 flex items-center gap-1">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={formData.phone}
                      onChange={handleInputChange}
                      className={`w-full border-2 rounded-lg px-3 py-2.5 outline-none focus:ring-0 transition-all ${formErrors.phone ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-blue-500"}`}
                      placeholder="0123456789"
                    />
                    {formErrors.phone && (
                      <p className="text-sm text-red-600 mt-1 flex items-center gap-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="relative">
                    ...
                    <select
                      ...
                      value={formData.role}
                      onChange={handleInputChange}
                      className={`w-full appearance-none border-2 rounded-lg px-3 py-2.5 pr-10 outline-none focus:ring-0 transition-all cursor-pointer ${formErrors.role ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-blue-500"}`}
                    >
                      <option value="">-- Chọn vai trò --</option>
                      <option value="STAFF_SALE">Nhân viên bán hàng</option>
                      <option value="LEAD_SALE">Trưởng nhóm bán hàng</option>
