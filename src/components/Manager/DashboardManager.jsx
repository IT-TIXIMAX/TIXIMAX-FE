...
const useCountAnimation = (
  targetValue,
  { shouldAnimate = true, duration = 800, decimals = 0 } = {},
) => {
  const [displayValue, setDisplayValue] = useState(Number(targetValue) || 0);
  const prevTargetRef = useRef(Number(targetValue) || 0);
  const animationRef = useRef(null);
...
const DashboardManager = () => {
  ...
  const [orders, setOrders] = useState({ newOrderLinks: 0, newOrders: 0 });
  const [customers, setCustomers] = useState({ newCustomers: 0 });
  ...
  const fetchDetails = async (currentFilter, silent = false) => {
    ...
    try {
      ...
      if (silent) setTimeout(() => setAnimationKey((prev) => prev + 1), 50);
      setHasLoadedDetailsOnce(true);
    } catch (err) {
      console.error(err);
      if (!silent) {
  ...
  useEffect(() => {
    if (!latestMessage || !isConnected) return;
    const ev = String(latestMessage?.event ?? "").toUpperCase();
    const shouldRefresh = [
      "DASHBOARD",
      "UPDATE",
    ...
    if (!shouldRefresh) return;
    if (wsTimerRef.current) clearTimeout(wsTimerRef.current);
    wsTimerRef.current = setTimeout(() => fetchDetails(filterType, true), 400);
    return () => {
      if (wsTimerRef.current) clearTimeout(wsTimerRef.current);
    };
  }, [latestMessage, isConnected, filterType]);
  ...
  const getFilterLabel = () =>
    FILTER_OPTIONS.find((t) => t.value === filterType)?.label || "Hôm nay";
  ...
  // ===== Shared card style =====
  const cardCls =
    "cursor-pointer bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-200 min-h-[150px]";
  const subCardCls =
    "cursor-pointer bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-200 min-h-[220px]";
      ...
      <div className="mx-auto p-4 md:p-6 lg:p-8">
        ...
        >
          ...
          >
            ...
            {isConnected && (
              <div
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-green-500"
            ...
            {refreshingDetails && (
              <div
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-500"
                title="Đang cập nhật..."
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
              {[
                ...
                },
                {
                  ...
                  suffix: " kg",
                  icon: Scale,
                  to: "/manager/dashboard/weight",
                },
                {
                  label: "Khối lượng thu",
                  value: stats.totalNetWeight,
                  suffix: " kg",
                  icon: Scale,
                  to: "/manager/dashboard/weight",
                },
              ].map(({ label, value, suffix, icon: Icon, to }) => (
                <div
                  key={label}
                  onClick={() => navigate(to)}
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => handleCardKeyDown(e, to)}
                  className={cardCls}
                >
                  <div className="p-5 md:p-6 bg-gradient-to-br from-gray-50 to-gray-100 h-full">
                    <div className="flex items-center justify-between mb-4 min-w-0">
                      <span className="text-gray-500 text-xs font-semibold uppercase tracking-wide truncate">
                        {label}
                      </span>
                      <div className="p-2.5 rounded-lg bg-white border border-gray-200 shrink-0">
        ...
        <div>
          ...
          {!hasLoadedDetailsOnce && loadingDetails ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              ...
              >
                <div className="p-5 md:p-6 bg-gradient-to-br from-yellow-100 to-yellow-200 h-full">
                  <div className="flex items-center justify-between mb-4 min-w-0">
                    <span className="text-yellow-800 text-xs font-semibold uppercase tracking-wide truncate">
                      Cân nặng
                    </span>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-baseline gap-2 min-w-0">
                      ...
                      <span className="text-sm text-yellow-800 font-medium shrink-0">
                        kg
                      </span>
                    </div>
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <span className="text-sm font-bold text-yellow-900 whitespace-nowrap">
                            <AnimatedNumber
                              key={`weight-total-d-${animationKey}`}
                              value={weights.totalWeight}
                              decimals={2}
                              suffix=" kg"
                              shouldAnimate={shouldAnimate}
              ...
              <div
                ...
                className={subCardCls}
              >
                <div className="p-5 md:p-6 bg-gradient-to-br from-red-100 to-red-200 h-full">
                  <div className="flex items-center justify-between mb-4 min-w-0">
                    <span className="text-red-800 text-xs font-semibold uppercase tracking-wide truncate">
                      Thanh toán
                    </span>
                    <div className="p-2.5 rounded-lg bg-red-500 shrink-0">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-baseline gap-2 min-w-0">
                      ...
                      <span className="text-sm text-red-800 font-medium shrink-0">
                        đ
                      </span>
                    </div>
                    <div className="pt-3 border-t border-red-300">
                      <div className="grid grid-cols-1 gap-2">
                        <div className="flex items-center justify-between gap-3">
                          <span className="text-xs text-red-800 font-medium truncate">
                            Tổng tiền thu
                          </span>
                          <span className="text-sm font-bold text-red-900 whitespace-nowrap">
                            <AnimatedNumber
                              key={`payment-col-d-${animationKey}`}
                              value={payments.totalCollectedAmount}
                              suffix=" đ"
                              shouldAnimate={shouldAnimate}
                            />
              ...
              <div
                ...
                className={subCardCls}
              >
                <div className="p-5 md:p-6 bg-gradient-to-br from-emerald-100 to-emerald-200 h-full">
                  <div className="flex items-center justify-between mb-4 min-w-0">
                    <span className="text-emerald-800 text-xs font-semibold uppercase tracking-wide truncate">
                      Đơn hàng
                    </span>
                    <div className="p-2.5 rounded-lg bg-emerald-600 shrink-0">
                    ...
                    <div className="pt-3 border-t border-emerald-300">
                      <div className="grid grid-cols-1 gap-2">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <span className="text-sm font-bold text-emerald-900 whitespace-nowrap">
                            <AnimatedNumber
                              key={`order-links-${animationKey}`}
                              value={orders.newOrderLinks}
                              shouldAnimate={shouldAnimate}
              ...
              <div
                ...
                className={subCardCls}
              >
                <div className="p-5 md:p-6 bg-gradient-to-br from-blue-100 to-blue-200 h-full flex flex-col">
                  <div className="flex items-center justify-between mb-4 min-w-0">
                    <span className="text-blue-800 text-xs font-semibold uppercase tracking-wide truncate">
                      Khách hàng mới
                    </span>
                    <div className="p-2.5 rounded-lg bg-blue-600 shrink-0">
