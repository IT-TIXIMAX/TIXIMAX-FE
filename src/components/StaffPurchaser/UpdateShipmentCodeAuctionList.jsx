...
const UpdateShipmentCodeAuctionList = () => {
  ...
  useEffect(() => {
    setPage(0);
  }, [size, debouncedQ, shipmentStatusFilter]);
  ...
  const items = useMemo(() => {
    ...
    if (!s) return list;
    return list.filter((it) => {
      const customer = getCustomerInfo(it);
      const inOrder =
        it.orderCode?.toLowerCase().includes(s) ||
        it.staffName?.toLowerCase().includes(s) ||
        it.purchaseCode?.toLowerCase().includes(s) ||
        customer?.name?.toLowerCase().includes(s) ||
        customer?.customerCode?.toLowerCase().includes(s);
        ...
        <div className="bg-blue-600 rounded-xl shadow-sm p-5 mb-6">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-semibold text-white">
                  Auction Shipment Code Management
                </h1>
              </div>
            </div>
            <button
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3 flex-shrink-0">
                          {status && (
                            <span className="inline-flex items-center rounded-lg px-3 py-1.5 text-sm font-medium bg-yellow-100 text-black border border-yellow-300">
                              {purchaseStatusLabel(status)}
                            </span>
                          )}
                    ...
                    <div className="p-4">
                      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                        ...
                        {/* Right Column - Product Links + Info */}
                        <div className="lg:col-span-3 space-y-4">
                          {/* Product Links */}
                          <div>
                            <div className="mb-2">
                                ...
                                {p.finalPriceOrder && (
                                  <div className="flex justify-between">
                                    ...
                                    <span className="font-medium text-green-600">
                                      {p.finalPriceOrder.toLocaleString()}đ
                                    </span>
                                  </div>
                                )}
                                {p.note && (
