...
const ShippingAddressList = () => {
  ...
  useEffect(() => {
    const emptyAll = !searchCustomerCode.trim() && !searchShipmentCode.trim();
    const hasFilter = !!(filterCustomerCode || filterShipmentCode);
  ...
  const handleSearch = useCallback(() => {
    // nếu searchCustomerCode đang là "CODE - Name" thì lấy CODE
    const raw = (searchCustomerCode || "").trim();
    const customerCode = raw ? raw.split(" - ")[0].trim() : "";

    setFilterCustomerCode(customerCode);
    setFilterShipmentCode(searchShipmentCode.trim());
    setPage(0);
  }, [searchCustomerCode, searchShipmentCode]);

  // ✅ clear AccountSearch (và clear filter luôn)
