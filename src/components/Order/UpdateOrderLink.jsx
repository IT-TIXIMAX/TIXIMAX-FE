...
const UpdateOrderLink = ({ orderId, isOpen, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (isOpen && orderId) fetchOrderData();
  ...
  // ✅ Ưu tiên lấy message đúng từ BE { error, message, status }
  const getApiErrorMessage = (err) => {
    ...
    return err?.message || "Không thể cập nhật đơn hàng";
  };
  ...
  const handleLinkChange = (index, field, value) => {
    setFormData((prev) => {
      ...
      return { ...prev, orderLinks: newLinks };
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < formData.orderLinks.length; i++) {
      ...
      const qty = parseNumber(link.quantity, 0);
      if (qty <= 0) {
        toast.error(`Sản phẩm ${i + 1}: Số lượng phải lớn hơn 0`);
        return;
      }
    ...
    try {
      ...
      const dataToSend = {
        exchangeRate: parseNumber(formData.exchangeRate),
        checkRequired: !!formData.checkRequired,
        note: formData.note || "",
        orderLinks: formData.orderLinks.map((link) => ({
          ...link,
          quantity: parseInteger(link.quantity),
          priceWeb: parseNumber(link.priceWeb),
          shipWeb: parseNumber(link.shipWeb),
          purchaseFee: parseNumber(link.purchaseFee),
          extraCharge: parseNumber(link.extraCharge),
      ...
      console.error("Error updating order:", error);
      toast.error(getApiErrorMessage(error)); // ✅ ra đúng message BE
    } finally {
      setLoading(false);
    }
  };
