...
/* ===================== Skeletons (synced with blue table style) ===================== */
...
const TableSkeleton = ({ rows = 10 }) => (
  <div className="animate-pulse">
    <div className="h-12 bg-gray-100 rounded-t-xl border-b border-gray-200" />
    <div className="space-y-0">
      {Array.from({ length: rows }).map((_, i) => (
        <div
          key={i}
          className="bg-white border-b border-gray-200 p-4 flex items-center gap-4"
        >
...
const PackingAwaitList = () => {
  ...
  const handlePrevPage = () => page > 0 && setPage((p) => p - 1);
  ...
  const handleSelectAll = () => {
    if (selectedPackings.length === filteredOrders.length) {
      ...
      setSelectedPackings(filteredOrders.map((o) => o.packingId));
    }
  };
  ...
  const handleExportSelected = async () => {
    if (selectedPackings.length === 0) {
      toast.error("Please select at least one package to export!");
      return;
    }
    ...
    try {
      ...
      const fileName = `packings_export_${new Date().toISOString().split("T")[0]}.xlsx`;
      XLSX.writeFile(wb, fileName);

      toast.success(
        `Exported successfully: ${data.length} packages (${stt - 1} detail rows)!`,
      );
    } catch (error) {
      console.error("Export error:", error);
  ...
  const stats = useMemo(() => {
    ...
    const filtered = filteredOrders?.length || 0;
    const selected = selectedPackings?.length || 0;
    return { total, filtered, selected };
  }, [orders, filteredOrders, selectedPackings]);
  ...
  return (
    <div className="min-h-screen">
      <div className="mx-auto p-6">
        ...
        {/* Success/Error Assign */}
        {assignSuccess && (
          <div className="bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-xl flex items-center gap-2 mb-4">
            ...
            <div className="flex flex-col lg:flex-row gap-3">
              ...
              <div className="w-full lg:w-56">
                <select
                  ...
                  onChange={handlePageSizeChange}
                  disabled={loading}
                  className="w-full px-3 py-2.5 border-2 border-gray-300 rounded-lg outline-none focus:ring-0 focus:border-blue-500 transition-all disabled:bg-gray-100"
                >
                  {PAGE_SIZES.map((s) => (
              ...
              <div className="flex flex-wrap gap-2 justify-start sm:justify-end">
                ...
                >
                  {exportLoading ? (
                    ...
                    <>
                      <Download className="w-4 h-4" />
                      Export Excel ({selectedPackings.length})
                    </>
                  )}
                </button>
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        <div className="flex flex-wrap gap-1 max-w-[520px]">
                          {order.packingList && order.packingList.length > 0 ? (
                            <>
                              {order.packingList.slice(0, 2).map((item, i) => (
                                <span
                                  key={i}
                              ...
                              {order.packingList.length > 2 && (
                                <span className="inline-block px-2 py-1 bg-gray-200 text-gray-700 text-xs rounded-md font-semibold">
                                  +{order.packingList.length - 2}
                                </span>
                              )}
                            </>
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        {order.flightCode ? (
                          <span className="inline-flex items-center gap-2 px-2.5 py-1 bg-blue-100 text-blue-700 text-xs rounded-full font-semibold border border-blue-200">
                            <span className="w-1.5 h-1.5 bg-blue-500 rounded-full" />
                            {order.flightCode}
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-2 px-2.5 py-1 bg-orange-100 text-orange-700 text-xs rounded-full font-semibold border border-orange-200">
            ...
            <div className="bg-white rounded-xl max-w-md w-full shadow-2xl overflow-hidden">
              ...
              <div className="p-6">
                <p className="text-sm text-gray-600 mb-4">
                  You are about to assign a flight to{" "}
                  <span className="font-semibold text-blue-700">
                    {selectedPackings.length}
                  </span>{" "}
                  selected packages.
