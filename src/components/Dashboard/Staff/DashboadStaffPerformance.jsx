...
import {
  getGoodsAndWeight,
  getSummaryStaff,
} from "../../../Services/Dashboard/staffdashboardService";
import FilterRoute from "../../Filter/FilterRoute";
...
const toStaffRows = (obj) =>
  Object.entries(obj || {}).map(([currency, v]) => ({
    ...
    totalParcels: Number(v?.totalParcels ?? 0),
  }));
...
const DashboadStaffPerformance = () => {
  ...
  const averageCompletionRate = useMemo(() => {
    ...
    const sum = staffRows.reduce((acc, r) => acc + r.completionRate, 0);
    return sum / staffRows.length;
  }, [staffRows]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [goodsRes, staffRes] = await Promise.all([
        getGoodsAndWeight(params),
        getSummaryStaff(params),
      ]);
      setData(goodsRes || {});
  ...
  return (
    <div className="min-h-screen p-6">
      <div className="mx-auto space-y-6">
        ...
        {loading ? (
          <LoadingSkeleton />
        ) : rows.length === 0 && staffRows.length === 0 ? (
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                    ...
                    <div className="text-3xl font-bold text-gray-900">
                      {formatNumber(staffTotals.totalOrders)}
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                    ...
                    <div className="text-3xl font-bold text-gray-900">
                      {formatNumber(staffTotals.completedOrders)}
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                    ...
                    <div className="text-3xl font-bold text-gray-900">
                      {formatNumber(staffTotals.totalParcels)}
                    </div>
                  </div>
                </div>
                    ...
                    <table className="w-full">
                          ...
                          >
                            ...
                            <td className="px-6 py-4 text-right">
                              <span className="text-base font-medium text-green-700">
                                {formatNumber(r.completedOrders)}
                              </span>
                            </td>
                            <td className="px-6 py-4 text-right">
                              <span
                      ...
                      <tfoot>
                        <tr className="bg-gray-50 border-t border-gray-200">
                          <td
                            colSpan="3"
                            className="px-6 py-4 font-medium text-gray-900 text-sm uppercase"
                          >
                            Tổng cộng
                          </td>
                          <td className="px-6 py-4 text-right">
